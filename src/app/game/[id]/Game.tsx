...
import {onValue,update,set,ref,getDatabase,serverTimestamp,onDisconnect, push} from "firebase/database";
...
export default function Game() {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(connRef, (s) => {
      ...
      onDisconnect(userRef).set(offline).then(() => set(userRef, online));
    });
...
useEffect(() => {
  ...
  const handleDrop = useCallback(
    (sourceSquare: any, targetSquare: any,thePiece:any) => {
      ...
      const move = clone.move({ from: sourceSquare, to: targetSquare, promotion: thePiece === "Q" ? "q" : thePiece === "N" ? "n" : thePiece === "B" ? "b" :"r",});
...
const gameRef = ref(db, `games/${params.id}/fen`);
  ...
  const Load=()=>{
    return (
      ...
      <main className={`transform absolute z-20  inset-x-0 ${(opening == 0) ? "-translate-y-full" : "translate-y-0 "} duration-300 ease-in-out flex items-center justify-center h-screen bg-blue-200 bg-opacity-25`}>
        ...
        <span className="lg:text-lg font-semibold select-none">Welcome to Pravin's Chess</span>
        </div>
      </main>
      </>
    )
  ...
  return (
    ...
    {Load()}
    <div
  className={`bg-blue-200 bg-opacity-25 h-screen py-3 gap-5 flex-col justify-center items-center
    transition-opacity duration-300 ${opening === 0 ? "opacity-100" : "opacity-0 pointer-events-none"}`}
>
  <div className="flex flex-col gap-5 lg:flex-row">

        <main className="lg:w-1/2">
          <div className="flex justify-center items-center gap-5 h-[40px]">
            <span
              className={
                !chess.game_over()
            ...
            >
              ...
              <span className="flex items-center gap-2">
               Game ID - {params.id} <svg onClick={()=>handleCopy(params.id)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4 text-red-500">
  <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z" />
        ...
        <main className="hidden lg:block lg:w-1/2">
            <ChatBox params={opponentId} decodedToken={opponentInfo} opponentOnline={opponentOnline} lastSeen={parseFloat(lastSeen)} />
        </main>


        <div className="my-5 lg:hidden mx-5">
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12.76c0 1.6 1.123 2.994 2.707 3.227 1.068.157 2.148.279 3.238.364.466.037.893.281 1.153.671L12 21l2.652-3.978c.26-.39.687-.634 1.153-.67 1.09-.086 2.17-.208 3.238-.365 1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0 0 12 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018Z" />
      ...
      <div className="flex justify-center">
        <button onClick={()=>{window.location.replace('/');sessionStorage.clear()}} className="text-white bg-red-500 p-1 px-2 rounded">Exit</button>
      </div>
      </div>
